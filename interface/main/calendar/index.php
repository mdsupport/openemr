...
$memcached = false;
// $req_begin = microtime (true);
if (($_SERVER['REQUEST_METHOD'] !== 'POST') && ($_GET['viewtype'] == 'day'))  {
    $memcached = 'cal_day';
    ...
    $m->setSaslAuthData('cacheuser', 'cacheuserpw');
    
    // Add server first time
    $ms = $m->getServerList();
    if ($ms[0]) {
        // echo $ms[0]['host']." has the cached info.";
    } else {
        // echo 'localhost added as memcached server.';
        $m->addServer('localhost', 11211);
    }
    $mk = $m->get($memcached);
    if ($mk) {
        // printf("Cached response in %dmS", (microtime(true) - $req_begin));
        echo $mk;
        exit;
    } else {
        // Content generated by original code will be cached 
    }
}
...
if ((empty($return)) || ($return == false)) {
    ...
    if ($memcached) {
        $buffer = ob_get_flush();
        echo $buffer;
        // $buffer = str_replace('Today', (microtime(true) - $req_begin), $buffer);
        // As a test, the cache will expire in 1 minute
        $m->set($memcached, $buffer, 60);
    }
} else {
    // duh?
}
